...
const Personalized = () => {
  ...
  useEffect(() => {
    try {
      const service = JSON.parse(localStorage.getItem("selectedService")) || { name: "", price: 0 };
      const clothes = JSON.parse(localStorage.getItem("selectedClothes")) || [];
      const fragrance = JSON.parse(localStorage.getItem("selectedFragrance")) || null;
      const packaging = JSON.parse(localStorage.getItem("selectedPackage")) || null;

      setOrder({
        serviceType: service,
        garments: clothes,
        fragrance: fragrance ? { ...fragrance, selected: true } : { selected: false },
        packaging: packaging
          ? {
              ...packaging,
              selected: true,
              description: packagingOptions.find((p) => p.id === packaging.id)?.description || "",
            }
          : { selected: false },
      });

      // hydrate the form if user typed earlier
      const existingMsg = JSON.parse(localStorage.getItem("cardMessage"));
      if (existingMsg) setFormData({ from: existingMsg.from || "", to: existingMsg.to || "" });
    } catch (err) {
  ...
  const getServicePrice = () => (order.serviceType?.price ? parsePrice(order.serviceType.price) : 0);

  const calculateGarmentsTotal = () =>
    order.garments.reduce((total, item) => {
      const price = parsePrice(item.price);
      const quantity = Math.max(1, Number(item.quantity) || 1);
      return total + price * quantity;
    }, 0);

  const calculateSubtotal = () => getServicePrice() + calculateGarmentsTotal();

  const calculateTotal = () => {
    const subtotal = calculateSubtotal();
    const fragrancePrice = order.fragrance?.selected ? parsePrice(order.fragrance.price || 0) : 0;
    const packagingPrice = order.packaging?.selected ? parsePrice(order.packaging.price || 0) : 0;
  ...
  // SAVE AS YOU TYPE so the requirement is tracked immediately
  const handleInputChange = (e) => {
    const { name, value } = e.target;
    setFormData((prev) => {
      const next = { ...prev, [name]: value };
      try { localStorage.setItem("cardMessage", JSON.stringify(next)); } catch {}
      return next;
    });
    if (name === "from") setErrors((prev) => ({ ...prev, from: value.trim() ? false : true }));
  };
  ...
  // ✅ condition to show Book Now button (only when "From" is filled)
  const canBookNow = formData.from.trim().length > 0;
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="bg-white rounded-xl shadow-lg overflow-hidden">
              ...
              <form onSubmit={handleSubmit} className="p-4 sm:p-6 lg:p-8">
                ...
                <div className="mt-6 sm:mt-8 flex flex-col sm:flex-row justify-between gap-3 sm:gap-0">
                  ...
                  {/* keeping your original: no submit CTA here */}
                </div>
              </form>
            </div>
          </div>
          ...
          <div className="lg:col-span-1">
            <div className="bg-white rounded-xl shadow-lg sticky top-20 max-h-[85vh] overflow-hidden flex flex-col">
                      ...
                      {order.garments.map((item) => (
                        <div key={item.name} className="bg-gray-50 rounded-lg p-2">
                          <div className="flex justify-between items-start mb-1">
                            <span className="text-xs font-medium truncate pr-2 flex-1">{item.name}</span>
                            <button
                          ...
                          <div className="flex justify-between items-center">
                            ...
                            <span className="text-xs font-medium">
                              {(parsePrice(item.price) * item.quantity).toFixed(2)} QAR
                            </span>
                          </div>
                        </div>
                      ))}
                  ...
                  {order.fragrance?.selected && (
                    <div className="border-b pb-2">
                      <div className="flex justify-between items-center">
                        ...
                        <span className="text-sm font-medium">+{parsePrice(order.fragrance.price).toFixed(2)} QAR</span>
                      </div>
                      <div className="text-xs text-gray-600 mt-1">{order.fragrance.name}</div>
                    </div>
                  )}
                  ...
                  {order.packaging?.selected && (
                    <div className="border-b pb-2">
                      <div className="flex justify-between items-center">
                        ...
                        <span className="text-sm font-medium">+{parsePrice(order.packaging.price).toFixed(2)} QAR</span>
                      </div>
                      <div className="text-xs text-gray-600 mt-1">
                        {order.packaging.name}
                        {order.packaging.description && <p className="text-gray-500">{order.packaging.description}</p>}
                      </div>
                    </div>
                  )}
              ...
              {/* Footer with Total + conditional Book Now */}
              <div className="border-t border-gray-200 p-4 sm:p-6 bg-gray-50">
                ...
                {canBookNow && (
                  <button
                    type="button"
                    className="w-full px-4 py-3 rounded-lg font-semibold text-black bg-[#D4AF37] hover:bg-[#c9a227] transition-all"
